...
- `implementer` — executes one implement pass per dispatch. Contract: changed files, validation results, residual risks or blockers, `Pass result: compliant | incomplete | invalid`, and whether the approved scope is fully complete.
...
The `implementer` fixes the same root-cause issue across the whole approved scope and does not revert unrelated edits. If its `Pass result` is `incomplete` or `invalid`, run a compliant replacement implement pass before review.

After each review pass that returns findings, append each addressed root cause to the plan's `Fixed root causes` section (see [TEMPLATES.md](TEMPLATES.md)): a stable `root-cause-id`, a one-line description, and the search pattern smell-detector will use later.

Loop safety valve: track consecutive failed rework passes deterministically with the `rework-tracker` skill. Before each re-dispatch of the `implementer` against the same root cause, read the implementer's `Pass result` (`compliant` | `incomplete` | `invalid`) and pass it to `rework-tracker` with the root-cause id; it returns `continue` or `stop`. Do not remap or reinterpret the implementer's `Pass result`. On `stop` (three consecutive `incomplete`/`invalid` against the same root cause), halt auto-looping and report the blocker to the user with the last attempts summarized. Resume only after user input. Do not spin indefinitely on a stuck rework. A `compliant` pass or a change of root cause resets the counter.
...
Before moving to Phase 5, run the `smell-detector` skill once. Use the search patterns recorded in the plan's `Fixed root causes` section (accumulated during Phase 4). If that section is empty — the loop reached `No findings` on the first implement pass with no prior findings — skip smell-detector, emit `Smell matches (outside approved scope)\n- none`, and proceed to Phase 5. Otherwise scan the whole repository for each pattern outside the approved scope and list matches as residual risks for the plan's `Risks` field. Matches are warnings, not blockers: they do not re-enter the Phase 4 loop and do not block delivery. Surface them to the user and proceed.
