...
`Fixed root causes` accumulates during Phase 4. After each review pass that returns findings, append one entry per addressed root cause:

```text
- <root-cause-id>: <one-line description> — pattern: <regex or token for smell-detector>
```

Leave this section empty when the loop reaches `No findings` on the first implement pass with no prior findings; smell-detector skips in that case.
