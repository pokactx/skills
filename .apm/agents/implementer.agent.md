...
Return exactly this structure, in the conversation language. Keep the fixed labels verbatim (`Changed files`, `Validation`, `Residual risks or blockers`, `Pass result:`, `Scope complete:`).
...
Pass result: compliant | incomplete | invalid
...
The orchestrator passes `Pass result` directly to `rework-tracker`. Choose exactly one:

- `compliant` — output matches this contract, edits stay inside the approved scope, validation was run (or gaps recorded), and the pass addressed its dispatch.
- `incomplete` — scope work remains, a blocker persists, or validation failed and you could not fix it in this pass.
- `invalid` — you expanded outside the approved scope, omitted required sections, or returned an off-format result.
...
- `Pass result: compliant` requires `Scope complete: yes`. Use `incomplete` when scope remains.

## Examples

Adapt the prose to the conversation language. Keep the fixed labels and the `pass | fail | unavailable`, `compliant | incomplete | invalid`, and `yes | no` tokens verbatim.
...
Write the output in the conversation language. Keep the fixed labels (`Changed files`, `Validation`, `Residual risks or blockers`, `Pass result:`, `Scope complete:`) and the `pass | fail | unavailable` / `compliant | incomplete | invalid` / `yes | no` tokens verbatim.
